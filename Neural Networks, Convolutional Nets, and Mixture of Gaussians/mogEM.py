...
def mogEM(x, K, iters, randConst=1, minVary=0):
    ...
    #Uncomment this K if running q2, comment if funning q4
    #K = 7
    iter_kmeans = 5
    #Uncomment for Kmeans initialization
...
def q2():
    ...
    #------------------- Add your code here ---------------------
    p, mu, vary, log_likelihood = mogEM(inputs_train, K, iters, randConst=randConst, minVary=minVary)
...
def q4():
    ...
    for t in xrange(T):
        ...
        #Training
        log_likelihood_train_angry = mogLogLikelihood(p_angry, mu_angry, vary_angry, x_train) + log_likelihood_class[0]
        log_likelihood_train_happy = mogLogLikelihood(p_happy, mu_happy, vary_happy, x_train) + log_likelihood_class[1]
        predict_label_train = (log_likelihood_train_angry < log_likelihood_train_happy).astype(float)

        #Validation
        log_likelihood_valid_angry = mogLogLikelihood(p_angry, mu_angry, vary_angry, x_valid) + log_likelihood_class[0]
        log_likelihood_valid_happy = mogLogLikelihood(p_happy, mu_happy, vary_happy, x_valid) + log_likelihood_class[1]
        predict_label_valid = (log_likelihood_valid_angry < log_likelihood_valid_happy).astype(float)

        #Test
        log_likelihood_test_angry = mogLogLikelihood(p_angry, mu_angry, vary_angry, x_test) + log_likelihood_class[0]
        log_likelihood_test_happy = mogLogLikelihood(p_happy, mu_happy, vary_happy, x_test) + log_likelihood_class[1]
        predict_label_test = (log_likelihood_test_angry < log_likelihood_test_happy).astype(float)

        errorTrain[t] = np.sum((predict_label_train != y_train).astype(float)) / y_train.shape[0]
        errorValidation[t] = np.sum((predict_label_valid != y_valid).astype(float)) / y_valid.shape[0]
        errorTest[t] = np.sum((predict_label_test != y_test).astype(float)) / y_test.shape[0]
