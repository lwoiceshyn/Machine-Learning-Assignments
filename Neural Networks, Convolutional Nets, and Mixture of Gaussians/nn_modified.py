...
#This script is used to look at probability distributions of examples under certain models
"""
Instruction:
...
def PlotProbabilities(softmax, targets):
     plt.xticks([1, 2, 3, 4, 5, 6, 7], ['Anger', 'Disgust', 'Fear', 'Happy', 'Sad', 'Surprise', 'Neutral'])
     plt.bar([1, 2, 3, 4, 5, 6, 7], softmax,align='center')
     plt.axis([0.5, 7.5, 0, 1.2])
     plt.title('Probability distribution for a sample {}'.format(targets))
...
def InitNN(num_inputs, num_hiddens, num_outputs):
    ...
    model = {
        ...
        'b2': b2,
        'b3': b3,
        'v_dW1': np.zeros((num_inputs, num_hiddens[0])),
        'v_dW2': np.zeros((num_hiddens[0], num_hiddens[1])),
        'v_dW3': np.zeros((num_hiddens[1], num_outputs)),
        'v_db1': np.zeros((num_hiddens[0])),
        'v_db2': np.zeros((num_hiddens[1])),
...
def AffineBackward(grad_y, x, w):
    ...
    """
    ###########################
    grad_x = np.dot(grad_y, w.T)  # 10 x 32
    grad_w = np.dot(x.T,grad_y) # 32 x 7
    grad_b = np.sum(grad_y,0) # 10 x 7
...
def main():
    ...
    # Train model.
    #stats = Train(model, NNForward, NNBackward, NNUpdate, eps,momentum, num_epochs, batch_size)
